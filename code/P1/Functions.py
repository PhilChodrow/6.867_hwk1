...
def GaussianFunc(x,gaussMean,gaussCov):
    ...
    f           =       -(1/math.sqrt(math.pow((2*math.pi), 2)*det))*math.exp(-0.5*(np.dot(np.dot(XlessU.T,linalg.inv(NPgaussCov)),XlessU)))
    return f
...
def SSE(theta,X,Y):
    return np.dot((np.dot(np.array(X),np.array(theta))-np.array(Y)).T,(np.dot(np.array(X),np.array(theta))-np.array(Y)))
